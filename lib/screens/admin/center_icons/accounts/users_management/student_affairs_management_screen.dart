...
class _StudentAffairsManagementScreenState extends State<StudentAffairsManagementScreen> {
  ...
  Future<void> _loadUsers() async {
    ...
    try {
      if (widget.mode == 'delete') {
        final all = await AdminServices().getUsers(roleId: 6, status: 'active', all: true);
        if (all != null) {
          setState(() {
            users = all;
          });
        ...
        if (pending != null) {
          setState(() {
            users = pending.where((u) => u['role_id'] == 6 || u['role']?['name'] == 'affairs').toList();
          });
        }
      }
    } catch (e) {
      debugPrint("Error loading affairs: $e");
    } finally {
  ...
  Future<void> _onCreateAffairs() async {
    ...
    try {
      ...
      if (success) {
        _showSnackBar("تم تسجيل موظف الشؤون بنجاح", isError: false);
        ...
        _showSnackBar("فشل إضافة موظف الشؤون");
      }
    } catch (e) {
      debugPrint("Error creating affairs: $e");
  ...
  Future<void> _onDeleteAffairs(int id) async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text("تأكيد الحذف"),
        content: const Text("هل تريد حذف حساب الموظف هذا نهائياً؟"),
        actions: [
    ...
    try {
      ...
      if (success) {
        _showSnackBar("تم حذف حساب الموظف بنجاح", isError: false);
        _loadUsers();
      } else {
  ...
  Future<void> _onApproveRequest(int id) async {
    ...
    try {
      ...
      if (success) {
        _showSnackBar("تم قبول وتفعيل حساب موظف الشؤون بنجاح", isError: false);
        _loadUsers();
      } else {
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text(widget.mode == 'create'
              ? 'إضافة موظف شؤون جديد'
              : widget.mode == 'delete'
                  ? 'إزالة موظف شؤون'
                  : 'طلبات الانضمام (موظفو الشؤون)'),
          centerTitle: true,
        ),
        body: isSubmitting
  ...
  Widget _buildDeleteView(bool isDark) {
    ...
    if (users.isEmpty) return const Center(child: Padding(padding: EdgeInsets.only(top: 50), child: Text("لا يوجد موظفو شؤون مسجلين")));
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        final affairs = users[index];
        final name = affairs['full_name'] ?? affairs['name'] ?? '';
        final username = affairs['username'] ?? '';
        final id = affairs['user_id'] ?? affairs['id'] ?? 0;
  ...
  Widget _buildRequestsList(bool isDark, Color cardColor) {
    if (isLoading) return const Center(child: CircularProgressIndicator());
    if (users.isEmpty) return const Center(child: Padding(padding: EdgeInsets.only(top: 50), child: Text("لا توجد طلبات معلقة من موظفي الشؤون")));
