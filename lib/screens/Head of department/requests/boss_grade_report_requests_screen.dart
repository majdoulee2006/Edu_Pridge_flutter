...
class BossGradeReportRequestsScreen extends StatefulWidget {
  const BossGradeReportRequestsScreen({super.key});

  @override
  State<BossGradeReportRequestsScreen> createState() => _BossGradeReportRequestsScreenState();
}

class _BossGradeReportRequestsScreenState extends State<BossGradeReportRequestsScreen> {
  ...
  Future<void> _fetchRequests() async {
    ...
    final data = await _apiService.getHeadGradeReportRequests();
    if (mounted) {
      setState(() {
        _requests = data ?? [];
        _isLoading = false;
              ...
              child: MediaQuery(
                ...
                child: Scaffold(
                  ...
                  appBar: AppBar(
                    ...
                    centerTitle: true,
                    title: Text(
                      isAr ? "طلبات تقارير العلامات" : "Grade Report Requests",
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Text(
                  req['course_name'] ?? 'مادة غير معروفة',
            ...
            Text(
              (isAr ? 'المعلم: ' : 'Teacher: ') + (req['teacher_name'] ?? ''),
              style: TextStyle(color: textColor.withOpacity(0.8), fontSize: 14),
            ),
            const SizedBox(height: 4),
            Text(
              (isAr ? 'حالة التقرير: ' : 'Status: ') + (req['status'] ?? 'pending'),
              style: TextStyle(color: Colors.grey.shade500, fontSize: 13),
            ),
          ],
        ),
